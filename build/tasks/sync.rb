...
class MyApp
  class Sync
    ...
    def get_git
      if @git == nil
        token = ENV['GH_TOKEN']
        FileUtils.mkdir_p MyApp.instance.get_temp_path
        FileUtils.cd(MyApp.instance.get_temp_path) do
    ...
    def copy_files
      ...
      FileUtils.cd(MyApp.instance.get_src_root + '/inc/language') do
        Dir.glob('**/*').each do |file|
          if MyApp.instance.match_path(file, match_files)
    ...
    def commit
      get_git.commit_all "Tagging: " + MyApp.instance.get_release_version
    end

    def tag
      get_git.lib.tag [MyApp.instance.get_release_version, '-f']
    end
